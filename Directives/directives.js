...
eLearningApp.directive('format', function() {
  return {
    ...
    link : function(scope, element, attrs, ngModel) {
      ...
      function read() {
        if(attrs.format === 'phone'){
          var newValue = phoneFormat(element.val());
        }
        else if(attrs.format === 'currency'){
            var newValue = currencyFormat(element.val());
        }
      ...
      function phoneFormat(number){
        ...
        else if(number.length === 1 || selectionStart === 1){
          return '('+number;
        }
      ...
      function currencyFormat(number){
          number = number.replace(/[^0-9]/g, '');
         if(number.length === 1 || selectionStart === 1 || (number.length <3 && number.length >0)){
          console.log('$format');
          return '$'+number;
        }
        else if(number.length >3){
